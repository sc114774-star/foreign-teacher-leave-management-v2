...
alter type public.foreign_teacher_leave_status add value if not exists 'Cancelled';
alter type public.foreign_teacher_notification_event add value if not exists 'Cancelled';
...
begin
  ...
  return jsonb_build_object('application_id', p_application_id, 'status', 'Cancelled', 'notification_id', notification_id);
end;
$$;
