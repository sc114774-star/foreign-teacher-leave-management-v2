...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: notification, error: notificationError } = await supabaseAdmin.from("foreign_teacher_leave_notifications").select("id, application_id, recipient_type, recipient_ref, event_type, status, foreign_teacher_leave_applications(application_no, leave_type, reason, start_at, end_at, teacher_id)").eq("id", body.notification_id).single();
    ...
    const application = notification.foreign_teacher_leave_applications as { teacher_id: string; application_no: string; leave_type: string; reason: string; start_at: string; end_at: string };
    ...
    await supabaseAdmin.from("foreign_teacher_leave_notifications").update({ status: "Sent", sent_at: new Date().toISOString(), error_message: null }).eq("id", notification.id);
    return json({ ok: true, status: "Sent" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    if (notificationId) await supabaseAdmin.from("foreign_teacher_leave_notifications").update({ status: "Failed", error_message: message }).eq("id", notificationId);
