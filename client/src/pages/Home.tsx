...
function toSupabaseLeaveRecord(application: SupabaseLeaveApplication): LeaveRecord {
  return toLeaveRecord({
    ...
    days: (application.leave_days ?? []).map((day) => ({ assignedSchool: day.assigned_school, routeReason: day.route_reason })),
    attachments: (application.leave_attachments ?? []).map((attachment) => ({ id: attachment.id, fileName: attachment.file_name, mimeType: attachment.mime_type, uploadedAt: attachment.uploaded_at })),
  });
}
        ...
        <div className="mx-auto max-w-[1440px] px-5 py-8 lg:px-10">
          ...
          {role === "teacher" ? <>
            ...
            <div className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
              <Card className="border-0 bg-white/85 shadow-[0_12px_35px_rgba(81,73,58,0.07)]"><CardHeader className="flex flex-row items-center justify-between pb-2"><div><CardTitle className="text-lg">Shared leave balance</CardTitle><p className="mt-1 text-sm text-[#92978f]">兩校共用額度 · Approved applications only</p></div><Badge variant="outline" className="rounded-full border-[#cbd9ca] bg-[#f5faf4] text-[#5e7a63]">Synced</Badge></CardHeader><CardContent className="space-y-5 pt-4">{balances.map((balance) => <div key={balance.label}><div className="mb-2 flex items-center justify-between text-sm"><span className="font-medium text-[#55635a]">{balance.label}</span><span className="text-[#879088]">{formatLeaveDaysBilingual(balance.used)} / {formatLeaveDaysBilingual(balance.total)}</span></div><div className="h-2 overflow-hidden rounded-full bg-[#edf0ea]"><div className={cn("h-full rounded-full", balance.color)} style={{ width: `${balance.used / balance.total * 100}%` }} /></div></div>)}<div className="mt-4 flex items-center gap-3 rounded-xl bg-[#f6f5ef] p-3 text-xs text-[#858b84]"><ShieldCheck className="h-4 w-4 text-[#78947c]" />額度於學校核准後正式扣除 · Balance is deducted after approval.</div></CardContent></Card>
...
function LeaveForm({ route, dateRange, setDateRange, calendarSettings, makeupDays, onClose, onSubmit }: { route: string; dateRange: { start: string; end: string }; setDateRange: (v: { start: string; end: string }) => void; calendarSettings: CalendarSettings; makeupDays: MakeupDay[]; onClose: () => void; onSubmit: (kind: "regular" | "makeup", makeupDate?: string, makeupSchool?: string, reason?: string, leaveType?: string, attachmentFile?: File, startTime?: string, endTime?: string) => void }) {
  ...
  const canSubmit = (!isMakeup || Boolean(configuredMakeup)) && (!attachmentRule.required || Boolean(attachmentFile));
...
function SchoolView({ records, role, active, calendarSettings, onSaveSettings, makeupDays, onSaveMakeup, onAction, onDecision, onPrint, onCollectionPrint, onPreviewAttachment }: { records: LeaveRecord[]; role: Role; active: string; calendarSettings: CalendarSettings; onSaveSettings: (settings: CalendarSettings) => void; makeupDays: MakeupDay[]; onSaveMakeup: (days: MakeupDay[]) => void; onAction: (message: string) => void; onDecision: (applicationId: number | undefined, school: string, decision: "Approved" | "Rejected") => void; onPrint: (record: LeaveRecord) => void; onCollectionPrint: (records: LeaveRecord[], title: string) => void; onPreviewAttachment: (attachment: AttachmentSummary) => void }) {
  ...
  if (active === "Settings") return <SchoolSettings role={role} calendarSettings={calendarSettings} onSaveSettings={onSaveSettings} makeupDays={makeupDays} onSaveMakeup={onSaveMakeup} onAction={onAction} />;
