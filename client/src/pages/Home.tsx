...
import { getSupabaseAttachmentUrl, fetchSupabaseLeaveApplications, createSupabaseLeaveApplication, uploadSupabaseLeaveAttachment, decideSupabaseLeaveApplication, fetchSupabaseMakeupDays, upsertSupabaseMakeupDay, deleteSupabaseMakeupDay, type SupabaseLeaveApplication } from "@/lib/supabaseLeave";
import {
  Bell,
  CalendarDays,
  Check,
...
import { fetchSupabaseLineGroupSettings, fetchSupabasePtoSettings, fetchSupabaseTeacherProfiles, upsertSupabaseLineGroupSetting, upsertSupabasePtoSetting } from "@/lib/supabaseLeave";
...
function calendarSettingsForToday(today = new Date()): CalendarSettings {
  ...
  return { contractStart: `${year}-08-01`, contractEnd: `${year + 1}-07-31`, vacationPeriods: [{ start: `${year}-07-01`, end: `${year}-08-31`, label: "Summer vacation" }, { start: `${year + 1}-01-21`, end: `${year + 1}-02-10`, label: "Winter vacation" }] };
}
...
function LeaveCollectionPrint({ records, title, academicYear, onClose }: { records: LeaveRecord[]; title: string; academicYear: string; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-[#f4f0e8] p-4 sm:p-8 print:static print:overflow-visible print:bg-white print:p-0">
    <div className="mx-auto max-w-[900px] bg-white p-8 text-[#1f2a26] shadow-xl print:max-w-none print:p-0 print:shadow-none">
      ...
      <div className="border-2 border-[#33443a]">
        <div className="border-b-2 border-[#33443a] p-5 text-center"><p className="text-lg font-bold tracking-[0.22em]">臺南市青山國民小學　{Number(academicYear.slice(0, 4)) - 1911} 學年度　請假卡</p><p className="mt-1 text-sm font-medium tracking-[0.1em]">{record.school}　{academicYear} Academic Year　Leave Application Form</p><p className="mt-2 text-[10px] text-[#6e776f]">依現行校務規範更新 · Current school leave policy</p></div>
...
export default function Home() {
  ...
  const [calendarSettings, setCalendarSettings] = useState<CalendarSettings>(() => { const current = calendarSettingsForToday(); const stored = readStored("foreign-teacher-calendar-settings", current); return academicYearForDate(stored.contractStart) === academicYearForDate(current.contractStart) ? stored : current; });
  const academicYear = academicYearForDate(calendarSettings.contractStart);
  const contractDays = Math.round((new Date(`${calendarSettings.contractEnd}T12:00:00`).getTime() - new Date(`${calendarSettings.contractStart}T12:00:00`).getTime()) / 86400000) + 1;
  ...
  const lineGroupSettingsQuery = useQuery({ queryKey: ["supabase", "line-group-settings"], queryFn: fetchSupabaseLineGroupSettings, enabled: role !== "teacher" && auth.supabaseConfigured, retry: false });
  const makeupQuery = useQuery({ queryKey: ["supabase", "makeup-days", academicYear, role], queryFn: () => fetchSupabaseMakeupDays(role === "teacher" ? undefined : role === "cingshan" ? "青山國小" : "東原國中"), enabled: Boolean(auth.user) && auth.supabaseConfigured, retry: false });
  ...
  const [dateRange, setDateRange] = useState({ start: defaultCalendarSettings.contractStart, end: defaultCalendarSettings.contractStart });
  const [makeupDays, setMakeupDays] = useState<MakeupDay[]>(() => readStored(`foreign-teacher-makeup-days-${role}`, defaultMakeupDays));
  ...
  useEffect(() => {
    if (makeupQuery.data) {
      const next = makeupQuery.data.map((day) => ({ date: day.makeup_date, sourceDate: day.source_date ?? undefined, school: day.assigned_school, note: day.note ?? undefined }));
      setMakeupDays((current) => JSON.stringify(current) === JSON.stringify(next) ? current : next);
    }
  }, [makeupQuery.data]);
  ...
  useEffect(() => {
    try {
      ...
      window.localStorage.setItem(`foreign-teacher-makeup-days-${role}`, JSON.stringify(makeupDays));
    } catch {
  ...
  const navItems = role === "teacher" ? [
    { key: "Dashboard", zh: "儀表板", Icon: LayoutDashboard }, { key: "Apply Leave", zh: "請假申請", Icon: Plus }, { key: "History", zh: "歷史紀錄", Icon: FileText },
  ...
  const navigateSection = (key: string) => { setActive(key); setMobileNav(false); if (role === "teacher" && key === "Apply Leave") setShowForm(true); };
  ...
  return <div className="relative min-h-screen overflow-hidden bg-[#f7f5f0] text-[#27352f]"><div aria-hidden="true" className="pointer-events-none absolute -left-24 top-24 h-[520px] w-[520px] rounded-full border border-[#dce7d8]/80" /><div aria-hidden="true" className="pointer-events-none absolute right-[-180px] top-[-140px] h-[620px] w-[620px] rotate-12 border border-[#e6dcca]/80" /><div aria-hidden="true" className="pointer-events-none absolute bottom-[-220px] left-[38%] h-[460px] w-[460px] -rotate-12 border border-[#dbe6e0]/70" />
    <div className="flex min-h-screen">
      <aside className={cn("fixed inset-y-0 left-0 z-40 w-[272px] border-r border-[#e8e3d9] bg-[#fbfaf7] px-5 py-6 transition-transform lg:static lg:translate-x-0 print:hidden", mobileNav ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex items-center gap-3 px-2"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-[#78a889] to-[#4f789d] text-white shadow-lg shadow-[#78a889]/20"><GraduationCap className="h-6 w-6" /></div><div><p className="font-semibold tracking-tight">休假系統</p><p className="text-[11px] uppercase tracking-[0.16em] text-[#9a9a90]">Leave System</p></div></div>
        ...
        <nav className="mt-8 space-y-1">{navItems.map(({ key, zh, Icon }) => <button key={key} onClick={() => navigateSection(key)} className={cn("flex w-full items-center justify-between rounded-xl px-3 py-3 text-left text-sm transition", active === key ? "bg-[#304b3b] text-white shadow-md shadow-[#304b3b]/10" : "text-[#7e847c] hover:bg-[#f0eee8] hover:text-[#304b3b]")}><span className="flex items-center gap-3"><Icon className="h-4 w-4" /><span>{zh}<span className={cn("ml-2 text-xs", active === key ? "text-white/65" : "text-[#adb0a8]")}>{key}</span></span></span>{active === key && <ChevronRight className="h-4 w-4" />}</button>)}</nav>
        <div className="mt-auto hidden border-t border-[#e8e3d9] pt-5 lg:block">{role !== "teacher" && <button onClick={() => setActive("Settings")} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm text-[#8a8d86] hover:bg-[#f0eee8]"><ShieldCheck className="h-4 w-4" />管理設定 · Settings</button>}<button onClick={async () => { await auth.logout(); window.location.href = "/login"; }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm text-[#8a8d86] hover:bg-[#f0eee8]"><LogOut className="h-4 w-4" />登出 · Sign out</button></div>
      </aside>
      ...
      <main className="min-w-0 flex-1 print:hidden">
        ...
        <div className="mx-auto max-w-[1440px] px-5 py-8 lg:px-10">
          ...
          {role === "teacher" ? <>
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard icon={Clock3} label="PTO remaining" value={`${balanceSummary.ptoRemaining} 日 / days`} helper={`已申請 ${balanceSummary.ptoUsed} 日 · Applied`} tone="bg-[#e3eee1] text-[#59765d]" /><MetricCard icon={CalendarDays} label="Contract period" value={`${contractDays} 日 / ${contractDays} days`} helper={`合約期間 · ${formatDate(calendarSettings.contractStart)} — ${formatDate(calendarSettings.contractEnd)}`} tone="bg-[#e9e5f2] text-[#72628f]" /><MetricCard icon={FileCheck2} label="Pending review" value={String(records.filter((record) => record.status === "Pending").length).padStart(2, "0")} helper="Awaiting school action" tone="bg-[#f4ead4] text-[#97773d]" /><MetricCard icon={Bell} label="Leave records" value={String(records.length).padStart(2, "0")} helper={`${academicYear} · Academic year`} tone="bg-[#f2e2dc] text-[#a05d4d]" /></div>
    ...
    {printRecord && <PrintLayout records={[printRecord]} academicYear={academicYear} title="請假卡 · Leave Application Form" onClose={() => setPrintRecord(null)} />}
    {printCollection && <PrintLayout records={printCollection.records} academicYear={academicYear} title={printCollection.title} onClose={() => setPrintCollection(null)} />}
...
function SchoolSettings({ role, academicYear, ptoSettings, teacherProfiles, lineGroupSettings, onSaveLineGroup, onSavePto, calendarSettings, onSaveSettings, makeupDays, onSaveMakeup, onAction }: { role: Role; academicYear: string; ptoSettings: import("@/lib/supabaseLeave").SupabasePtoSetting[]; teacherProfiles: import("@/lib/supabaseLeave").SupabaseTeacherProfile[]; lineGroupSettings: import("@/lib/supabaseLeave").SupabaseLineGroupSetting[]; onSaveLineGroup: (school: "青山國小" | "東原國中", groupId: string) => Promise<void>; onSavePto: (teacherId: string, totalDays: number) => Promise<void>; calendarSettings: CalendarSettings; onSaveSettings: (settings: CalendarSettings) => void; makeupDays: MakeupDay[]; onSaveMakeup: (days: MakeupDay[]) => void; onAction: (message: string) => void }) {
...
function SchoolView({ records, role, active, academicYear, ptoSettings, teacherProfiles, lineGroupSettings, onSaveLineGroup, onSavePto, calendarSettings, onSaveSettings, makeupDays, onSaveMakeup, onAction, onDecision, onPrint, onCollectionPrint, onPreviewAttachment }: { records: LeaveRecord[]; role: Role; active: string; academicYear: string; ptoSettings: import("@/lib/supabaseLeave").SupabasePtoSetting[]; teacherProfiles: import("@/lib/supabaseLeave").SupabaseTeacherProfile[]; lineGroupSettings: import("@/lib/supabaseLeave").SupabaseLineGroupSetting[]; onSaveLineGroup: (school: "青山國小" | "東原國中", groupId: string) => Promise<void>; onSavePto: (teacherId: string, totalDays: number) => Promise<void>; calendarSettings: CalendarSettings; onSaveSettings: (settings: CalendarSettings) => void; makeupDays: MakeupDay[]; onSaveMakeup: (days: MakeupDay[]) => void; onAction: (message: string) => void; onDecision: (applicationId: number | undefined, school: string, decision: "Approved" | "Rejected") => void; onPrint: (record: LeaveRecord) => void; onCollectionPrint: (records: LeaveRecord[], title: string) => void; onPreviewAttachment: (attachment: AttachmentSummary) => void }) {
  ...
  useEffect(() => {
    if (!availablePrintMonths.includes(printMonth)) setPrintMonth(currentYearMonth());
  ...
  if (active === "Settings") return <SchoolSettings role={role} academicYear={academicYear} ptoSettings={ptoSettings} teacherProfiles={teacherProfiles} lineGroupSettings={lineGroupSettings} onSaveLineGroup={onSaveLineGroup} onSavePto={onSavePto} calendarSettings={calendarSettings} onSaveSettings={onSaveSettings} makeupDays={makeupDays} onSaveMakeup={onSaveMakeup} onAction={onAction} />;
