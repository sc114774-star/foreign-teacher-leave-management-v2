...
export default function Home() {
  ...
  const navigateSection = (key: string) => { setActive(key); setMobileNav(false); if (role === "teacher" && key === "Apply Leave") { setShowForm(true); return; } const target = key === "History" ? "teacher-history" : key === "Leave Records" || key === "Pending Review" ? "school-records" : key === "Print Center" ? "school-print-center" : undefined; if (target) window.requestAnimationFrame(() => document.getElementById(target)?.scrollIntoView({ behavior: "smooth", block: "start" })); };
  ...
  return <div className="relative min-h-screen overflow-hidden bg-[#f7f5f0] text-[#27352f]"><div aria-hidden="true" className="pointer-events-none absolute -left-24 top-24 h-[520px] w-[520px] rounded-full border border-[#dce7d8]/80" /><div aria-hidden="true" className="pointer-events-none absolute right-[-180px] top-[-140px] h-[620px] w-[620px] rotate-12 border border-[#e6dcca]/80" /><div aria-hidden="true" className="pointer-events-none absolute bottom-[-220px] left-[38%] h-[460px] w-[460px] -rotate-12 border border-[#dbe6e0]/70" />
    ...
    {printRecord && <LeaveCardPrint record={printRecord} academicYear={academicYear} onClose={() => setPrintRecord(null)} />}
    {printCollection && <LeaveCollectionPrint records={printCollection.records} title={printCollection.title} academicYear={academicYear} onClose={() => setPrintCollection(null)} />}
